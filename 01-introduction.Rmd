...
> RStudio has lots of keyboard shortcuts to make coding quicker and easier.
Try to find the menu listing all the keyboard shortcuts, including the shortcut
to find the menu!
...
install as arguements.

Many, if not most, functions can take more than one arguement. The creators of the 
function should have given these defaults for the situation where the user provides
only one or some arguements. RStudio should prompt you for the arguements as you
...
For example, to find out all the arguements for `install.packages()` we'd
...
```{r list-index, fig.cap='(ref:list-index)', echo=FALSE}
...
strings as catergorical data using `stringsAsFactors = FALSE`
...
We deal with modifying data frames in later chapters.
