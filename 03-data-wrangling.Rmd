...
surveys_ic <- tibble(species_id = c("DM","DM",NA,"DS","DS","DS"),
                     mean_weight = c(3.75,2.50,8.50,7.50,NA,2.50))
surveys_ic
```

Replace `NA` with the last non-missing value, last observation carried forward.
...
Other strategies might be to impute values by using 
...
Let's repeat the analysis but having filled our table

```{r mk-krats, cache=TRUE}
krats <- surveys %>% 
