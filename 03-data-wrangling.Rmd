...
surveys_ms <- tibble(quarter = c(1991.1,1991.2,1991.3,1991.4,1992.1,1992.2,1992.4),
...
### Checking for explicit missing values using R
...
`sum()` to provide the total number of missing values for a variable or for 
all variables.

For a single variable, we'd extend our use of `select`:
...
Extending this a whole data frame requires introducing more syntax and the use
of a map function from the `purr` package. (Base R can also do this, but we're
staying in the tidyverse where possible). 
