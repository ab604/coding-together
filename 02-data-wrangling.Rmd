...
Let's also use some of their data to explore this question: **What is the effect 
of the exclusion of kangeroo rats from a plot of land on the granivore population?**
...
Concretely, it's a package of functions from the tidyverse.
...
> Ctl+Shift+M is a keyboard shortcut to create a pipe.

For the filter iteself, from R4DS:
...
for the observations only occuring on `plot_type`'s control or long term kangeroo 
rat exclusion. This time we assign the output to a new data frame called `surveys_filtered`.

**Note:** as `plot_type` is a charater vector we put the terms in quotes,
...
Or we could use `arrange()` to find the reocrd with the shortest hindfoot. 
*Note:* `arrange()` defualts to ascending order.
...
Often your data contains variables you don't need for the analysis you are performing.
...
arguement is the dataset, and the subsequent arguments are the new variables we wish
to create.
...
as to whether Kangeroo rats effect the size of the granivore population would
...
This allows us to pass diferent values to our new `rodent_type` variable **if**
they match either `species_id` values corresponding with Kangeroo rats or Granivores.
...
The first arguement to `case_when()` is the variable and value we want to match,
...
this condition. Here we want our new variable `rodent_type` to be "Kangeroo Rat".
...
to the rest by giving the arguement `TRUE` followed by the value. This means if
...
```{r dplyr-summarise, fig.cap='(ref:summarise)', fig.show='hold',fig.align='center', echo=FALSE,cache=TRUE}
...
```{r group-by, fig.cap='(ref:group-by)',fig.align='center', echo=FALSE,cache=TRUE}
...
## Using `dpylr` to explore the effect of Kangeroo Rats on Granivore populations

Let's use what we've learnt so far to explore the effect of Kangeroo Rats on Granivore populations
...
one way to visual this, comparing the observations between the control plots and the Kangeroo
rat exclusion plots.

One way to do this is to:

1. `filter()` the observations for the control and exlusion plots.
2.  Create a new `rodent_type` variable for Kangeroo Rats and Granivores.
...
It would be clearer to plot the data on three month (quartley) basis rather than
plotting every date in the dataset, so we need to create a variable that contains
the quarter in which the observation was made, for each observiation.
...
`quarter` variables. In other words we've grouped the data according to Kangeroo Rat or
