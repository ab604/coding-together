...
But for a brief overview, two questions one might immediately have are:
...
```{r viz_dat, echo=TRUE, cache=TRUE}
...
p4 <- viz_dat %>% ggplot(aes(Country,c(2,3,2.2,1.8,2.1), size = Percent, label = Country)) +
...
(ref:viz-plot) The same percentages for five Countries A-E is plotted seven ways to illustrate the differences in ease of making comparisons depending the plot type. The percentages are A:22%, B:25%, C:34%, D:29% and E:32% in all plots.

```{r viz-plot,fig.cap='(ref:viz-plot)', echo=FALSE, cache=T}
